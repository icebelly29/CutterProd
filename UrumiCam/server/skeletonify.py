...
def mergeFrags(c0, c1, sx, dr):
    for i in range(len(c1) - 1, -1, -1):
        if dr == HORIZONTAL:
            if mergeImpl(c0, c1, i, sx, False, 1):
                continue
            if mergeImpl(c0, c1, i, sx, False, 3):
                continue
            if mergeImpl(c0, c1, i, sx, False, 0):
                continue
            if mergeImpl(c0, c1, i, sx, False, 2):
                continue
        else:
            if mergeImpl(c0, c1, i, sx, True, 1):
                continue
            if mergeImpl(c0, c1, i, sx, True, 3):
                continue
            if mergeImpl(c0, c1, i, sx, True, 0):
                continue
            if mergeImpl(c0, c1, i, sx, True, 2):
                continue
        ...
        for i in range(y + 1, y + h - 1):
            for j in range(x + 1, x + w - 1):
                ...
                if s > ms:
                    ...
                    mj = j
                    ms = s
                elif s == ms and abs(j - (x + w // 2)) + abs(i - (y + h // 2)) < abs(
                    mj - (x + w // 2)
                ) + abs(mi - (y + h // 2)):
                    mi = i
                    mj = j
    ...
    if h > csize and mi != -1:  # split top and bottom
        ...
        if notEmpty(
            ...
            if rects != None:
                rects.append(L)
            nf += traceSkeleton(
                im, L[0], L[1], L[2], L[3], csize, maxIter - 1, rects
            )  # recurse
        ...
        if notEmpty(im, R[0], R[1], R[2], R[3]):
            ...
            mergeFrags(
                nf,
                traceSkeleton(im, R[0], R[1], R[2], R[3], csize, maxIter - 1, rects),
                mi,
                VERTICAL,
            )
