...
def api_method2_upload():
    ...
    try:
        ...
        # 61 block size prevents most hollowing, C=10 rejects shadows while catching faint ink
        binary_mask = cv2.adaptiveThreshold(blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 61, 10)

        # Clear out the border edges (ArUco frame) so it isn't skeletonized.
        # The ArUco markers and frame typically take up 5-8% of the image edge.
        mask_edge_margin = max(10, int(0.08 * min(w_px, h_px)))
